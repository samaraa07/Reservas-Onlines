...
def validar_horario_comercial(data_hora):
    """Valida se está dentro do horário comercial (8h-20h)"""
    hora = data_hora.hour
    ...
    if hora < 8 or hora >= 20:
        return False, "Horário fora do expediente (8h-20h)"
    
    return True, "OK"

def enviar_email(destinatario, assunto, corpo):
    print(f"E-mail simulado enviado para {destinatario}")
    return True
...
def login():
    if request.method == 'POST':
        ...
        if not user:
            flash("Usuário não encontrado. <a href='/register_cliente'>Cadastre-se aqui.</a>", "danger")
            return redirect(url_for('login'))
        if not check_password_hash(user.senha_hash, senha_digitada):
...
def relatorios():
    u = current_user()
    if u.perfil != 'admin':
        flash("Apenas administradores podem acessar relatórios.", "danger")
        return redirect(url_for('index'))
    ...
    receita_total = db.session.query(func.sum(Servico.preco)).join(
        Agendamento
    ).filter(Agendamento.status == 'confirmado').scalar() or 0
    ...
    prof_ranking = db.session.query(
        User.nome,
        Profissional.especialidades,
        func.count(Agendamento.id).label('total')
    ).join(Profissional).join(
    ...
    agendamentos_mensais = db.session.query(
        func.strftime('%Y-%m', Agendamento.data_hora).label('mes'),
        func.count(Agendamento.id).label('total')
    ).filter(Agendamento.data_hora >= seis_meses_atras).group_by('mes').all()
...
def reservar():
    ...
    if request.method == 'POST':
        ...
        valido, mensagem = validar_horario_comercial(dt)
        if not valido:
...
def servicos_por_profissional():
    ...
    return jsonify([{"id": s.id, "nome": s.nome, "duracao": s.duracao_min} for s in servicos])


# -------------------------
# NOVO: API para horários ocupados (BUG FIX)
# -------------------------
...
def horarios_ocupados():
    ...
    try:
        data_obj = parser.parse(data_str).date()
    except:
        return jsonify([])
    
    # Buscar agendamentos do profissional naquela data
    ...
    for a in ags:
        if a.data_hora.date() == data_obj:
            horarios.append(a.data_hora.strftime('%H:%M'))
...
def cancelar_agendamento(ag_id):
    ...
    # Determinar quem cancelou para personalizar a mensagem
    if u.perfil == 'profissional':
        cancelamento_mensagem = (
            f"Seu agendamento de {ag.servico.nome} com {ag.profissional.user.nome} "
            f"em {ag.data_hora.strftime('%d/%m/%Y %H:%M')} foi CANCELADO pelo profissional."
        )
        destinatario_notif = ag.cliente.user.id
    elif u.perfil == 'cliente':
        cancelamento_mensagem = (
            f"O cliente {ag.cliente.user.nome} cancelou o agendamento de {ag.servico.nome} "
            f"em {ag.data_hora.strftime('%d/%m/%Y %H:%M')}."
        )
        destinatario_notif = ag.profissional.user.id
    else:  # admin
        cancelamento_mensagem = (
            f"O agendamento de {ag.servico.nome} com {ag.profissional.user.nome} "
            f"em {ag.data_hora.strftime('%d/%m/%Y %H:%M')} foi CANCELADO pelo administrador."
        )
        destinatario_notif = ag.cliente.user.id

    nova_notif = Notificacao(user_id=destinatario_notif, mensagem=cancelamento_mensagem)

    try:
        db.session.add(nova_notif)
        db.session.commit()

        # -------- ENVIO DO E-MAIL CANCELADO PARA O CLIENTE
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                ...
                <div style="background: #f8d7da; border-left: 4px solid #dc3545; padding: 20px; border-radius: 8px; margin: 20px 0;">
                    <h3 style="color: #721c24; margin-top: 0;"> Detalhes do Agendamento Cancelado:</h3>
                    ...
                    <p style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #f5c6cb;">
                        <strong style="color: #721c24;">Status: CANCELADO</strong>
                    </p>
                </div>
                
                <p>Você pode fazer uma nova reserva a qualquer momento.</p>
                <p>Acesse o sistema: <a href="{request.url_root}reservar" style="color: #8B5E3C;">Fazer Nova Reserva</a></p>
        ...
        flash("Agendamento cancelado e notificação enviada.", "info")
    except Exception as e:
        db.session.rollback()
